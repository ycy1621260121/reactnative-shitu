...
import { Intellectual } from '../pages/Intellectual/index'
import { Intellectualmine } from '../pages/Intellectualmine/index'
import { Intellectualadd } from '../pages/Intellectualadd/index'
...
const MyTab = createBottomTabNavigator(
  {
		Home: {
		  screen: Home,
		  navigationOptions: () => TabOptions('商品', 'home')
		},
    ShiTu: {
      screen: ShiTu,
      navigationOptions: () => TabOptions('投票', 'shitu')
    },
		Message:{
			screen: Message,
			navigationOptions: () => TabOptions('消息', 'message')
		},
    News: {
      screen: News,
      navigationOptions: () => TabOptions('购物车', 'news')
    },
    Main: {
      screen: Main,
...
Main.navigationOptions = ({ navigation }) => {
  ...
  const tabBarButtonComponent = (props: any) => {
    return [
      <TouchableOpacity {...props} activeOpacity={1} style={{ width: SCREEN_WIDTH / 4 }} key={'tabBar'} />,
      <Badge count={badgeNumber} key={'Badge'} style={{ position: 'absolute', left: SCREEN_WIDTH - 40, top: 5 }} />
    ];
  };
...
export const AppRouter = createStackNavigator(
  {
    ...
    MainData: {
      screen: MainData
		...
		Intellectualmine:{
			screen:Intellectualmine
		},
		Intellectualadd:{
			screen:Intellectualadd
		}
  },
  {
  ...
  const tabBarIcon = ({ focused }: { focused: boolean }) => {
    ...
    return (
      <View style={{ marginTop: 3 }}>
        <CustomIcon name={tabBarIconName} size={28} color={color} />
      </View>
    );
  };
