...
@observer
export class Intellectual extends Component<Props, any> {
	  ...
	  let params = {
		...
		this._dataLoading = true;
		 let timer = setTimeout(async()=>{
		  clearTimeout(timer)
	  const intellectualData = await this.intellectualMobx.intellectual(params);
...
//       })
	  ...
	  const intellectualData = await this.intellectualMobx.intellectual(params);
	  console.log('here',this._dataSource.cloneWithRows(intellectualData.list))
	  if(intellectualData.code == 0){
		  this.setState({
...
_renderRow(rowData) {
	const right = [
	    ...
	    {
	      text: '我的创新',
	      onPress: () => this.props.navigation.navigate('Intellectualmine'),
...
// 	  ];
    return (
        <List>
          ...
          >
            <List.Item>
             {rowData.listId}{rowData.content}
            </List.Item>
          </SwipeAction>
        </List>
	  )
  ...
  render() {
    ...
    return (
      ...
      <BaseContainer style={styles.container} title={'商品知识产权'}>
				<SwRefreshListView
