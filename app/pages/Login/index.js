...
export class Login extends React.Component<Props, any> {
  ...
  login = async () => {
    ...
    this.codeRef.blur();
		var _self=this;
		let params = {
				username:this.loginMobx.params.mobile,
				password:this.loginMobx.params.code
			};
		const loginData = await this.loginMobx.login(params)
		//console.log(loginData)
		if(loginData.code == 0){
			await this.props.powerStore.setUserinfoData(loginData);
			await this.props.powerStore.setSessionToken(loginData.sessionToken);
			let sessionTokenData = {
					sessionToken:loginData.sessionToken,
				};
			const userinfoData = await _self.loginMobx.userInfo(sessionTokenData)
			//console.log(userinfoData);	
			if(userinfoData.code==0){
				await this.props.powerStore.setUsername(userinfoData.list.username);
				await this.props.powerStore.setHeadImg(userinfoData.list.headImg);
				this.props.navigation.navigate('Home');
			}else{
			Toast.show(userinfoData.message)
		}
			
		}else{
			Toast.show(loginData.message)
		}
		/*ajax({
			...
			success: (data) => {
				console.log(data)
				this.props.navigation.navigate('ShiTu')
			},
			error: (err) => {
				Toast.show({
						text: '网络请求异常',
        ...
        <ScrollView style={styles.scrollView} keyboardShouldPersistTaps={'always'}>
          ...
          <View style={styles.textInputBox}>
            ...
            <GradientButton
              ...
              title="登录"
            />
						<TouchableOpacity activeOpacity={1} onPress={this.toRegister} style={styles.sendCodeView}>
						<Text>去注册</Text>
						</TouchableOpacity>
          </View>
        </ScrollView>
