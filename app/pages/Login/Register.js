...
import { RegisterMobx } from '../../mobx/Register';
...
export class Register extends React.Component<Props, any> {
  ...
  constructor(props: Props) {
    super(props);
    this.registerMobx = new RegisterMobx();
    this.state = {
      sec: sec
    };
  }
  ...
  getVerifyCode = async () => {
    ...
    if (this.state.sec === sec) {
      ...
      const mobile = this.registerMobx.params['mobile'];
      ...
      try {
        await this.registerMobx.sendCode(this.registerMobx.params);
      } catch (e) {
  ...
  Register = async () => {
    ...
    this.codeRef.blur();
		let params = {
				username:this.registerMobx.params.mobile,
				password:this.registerMobx.params.code
			};
		
		const registerData = await this.registerMobx.register(params)
		if(registerData.code == 0){
			this.props.navigation.navigate('Login')
		}else{
			Toast.show(registerData.message)
		}
          ...
          <View style={styles.textInputBox}>
            <View style={[styles.textInputView, styles.phoneInputView]}>
              <TextInput
                ...
                onChangeText={text => {
                  this.registerMobx.setLoginData(text, 'mobile');
                }}
              />
          ...
          <View style={styles.textInputBox}>
            <View style={styles.textInputView}>
              <TextInput
                ...
                onChangeText={text => {
                  this.registerMobx.setLoginData(text, 'code');
                }}
              />
            </View>
            ...
            <GradientButton
              ...
              title="注册"
            />
						<TouchableOpacity activeOpacity={1} onPress={this.toLogin} style={styles.sendCodeView}>
						<Text>去登录</Text>
						</TouchableOpacity>
          </View>
        </ScrollView>
